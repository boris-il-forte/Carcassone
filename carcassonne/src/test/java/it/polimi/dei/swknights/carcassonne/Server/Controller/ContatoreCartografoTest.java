...
public class ContatoreCartografoTest
{
	...
	public void stradaPiccola() throws Exception
	{
		...
		assertTrue("Male! aspettavo 1 costruzione completata, viste: " + this.completate,
				this.completate == 1);
	...
	@Test
	public void cittaConSegnalini() throws Exception
	{
		FactoryTessere factory = new FactoryTessereNormali();
		...
		this.mettiContaESegnalino(factory, new Coordinate(3, 1), 1, PuntoCardinale.ovest); // est
		// this.mettiContaESegnalino(factory, new Coordinate(3, 1), 1,
		// PuntoCardinale.est);

		// assertTrue("punteggio errato", punteggiTurno.get(Color.green) == 10);
	...
	private void mettiContaESegnalino(FactoryTessere factory, Coordinate coord, int quanteCompleta,
			PuntoCardinale punto) throws MossaNonValidaException
	{

		Tessera tessera = factory.getTessera();
		Debug.print("" + tessera);
		Segnalino segnalino = new Segnalino(Color.red);
		...
		{
			this.completate++;
			this.contatorecartografo.getCostruzioniCompletate();
			tessera.setSegnalino(segnalino, punto);
			this.contatorecartografo.addSegnalino(segnalino, punto);

			Punteggi punteggiTurno = contatorecartografo.getPunteggioTurno();
			assertTrue("punteggio errato", punteggiTurno.get(Color.red) == 10);
		}

	}

	private void mettiSegnalinoEContaFine(FactoryTessere factory, Coordinate coord, int quanteCompleta,
			PuntoCardinale punto) throws MossaNonValidaException
	{
		...
		Punteggi punteggioFinale = contatorecartografo.getPunteggioFinale();
		assertTrue("punteggio errato, ottenuti: " +    punteggioFinale.get(Color.red) +
				"anzichè 5 ", punteggioFinale.get(Color.red) == 5);

	}

	@Test
	public void getPunteggioFinale() throws Exception
	{
		Debug.print(" punteggio ");
		FactoryTessere factory = new FactoryTessereNormali();
		factory.acquisisciMazzoDaFile("/Finale.txt");

		this.mettiEConta(factory, new Coordinate(1, 0), 0);
		this.mettiEConta(factory, new Coordinate(2, 0), 0);
		this.mettiEConta(factory, new Coordinate(3, 0), 0);
		this.mettiEConta(factory, new Coordinate(4, 0), 0);
		this.mettiSegnalinoEContaFine(factory, new Coordinate(5, 0), 0, PuntoCardinale.est);

		Punteggi punti = this.contatorecartografo.getPunteggioFinale();
		assertTrue(punti.getVincitoriAttuale().size() != 0);
		Debug.print("" + punti);
	...
	public void cittaMaledettaBassoAltoSerpe() throws Exception
	{
		...
		this.mettiEConta(factory, new Coordinate(2, 0), 0); // croce, risalgo da
															// dx

		this.mettiEConta(factory, new Coordinate(2, -1), 0); // croce risale da
																// dx
		this.mettiEConta(factory, new Coordinate(1, -1), 0); // strada in alto,
																// angolo strada
																// giù dx
		this.mettiEConta(factory, new Coordinate(0, -1), 0); // prato e città
																// sx, risalgo
																// da sx

		this.mettiEConta(factory, new Coordinate(0, -2), 0); // prato e città sx
		this.mettiEConta(factory, new Coordinate(1, -2), 0); // 3 città dx alto
																// basso, prato
																// sx
		this.mettiEConta(factory, new Coordinate(2, -2), 0); // città alto e sx
																// strada
																// giu-dx,
																// risalgo dx

		this.mettiEConta(factory, new Coordinate(2, -3), 0); // città sotto ,
																// strada EO,
																// prato sopra
		this.mettiEConta(factory, new Coordinate(1, -3), 0);
		this.mettiEConta(factory, new Coordinate(0, -3), 0);

		this.mettiEConta(factory, new Coordinate(0, -4), 0); // città alto, sx
																// prato basso e
																// dx
		this.mettiEConta(factory, new Coordinate(1, -4), 0);
		this.mettiEConta(factory, new Coordinate(2, -4), 0);

		this.mettiEConta(factory, new Coordinate(2, -5), 1); // città sotto
																// prato resto
		this.mettiEConta(factory, new Coordinate(1, -5), 1);
		this.mettiEConta(factory, new Coordinate(0, -5), 1); // strada NS, prato
																// resto
	...
	private void mettiEConta(FactoryTessere factory, Coordinate coord, int quanteCompleta) throws Exception
	{
		...
		if (this.contatorecartografo.areCostruzioniCompletate())
		{
			Debug.print(" metto la tessera alla coordinata:  " + coord + " la tessera e': " + tessera);
